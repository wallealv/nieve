...
import { createRegionalGridHandler } from '../../api/regional-grid.js';
...
describe('regional grid API', () => {
  ...
  test('does not accept arbitrary coordinates or methods', async () => {
    ...
    const response = await handler(new Request('https://example.com/api/regional-grid?latitude=0&longitude=0', { method: 'POST' }));
    expect(response.status).toBe(405);
  });

  test('returns a structured 503', async () => {
    const response = await createRegionalGridHandler(async () => { throw new Error('grid unavailable'); })(new Request('https://example.com/api/regional-grid'));

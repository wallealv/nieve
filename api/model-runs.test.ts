...
import { createModelRunsHandler } from './model-runs.js';
...
describe('model runs API', () => {
  ...
  test('rejects non-GET methods', async () => {
    const response = await createModelRunsHandler(async () => ({}))(
      new Request('https://example.com/api/model-runs', { method: 'POST' }),
    );
  ...
  test('returns a structured 503', async () => {
    ...
    })(new Request('https://example.com/api/model-runs'));
